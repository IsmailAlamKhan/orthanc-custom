...
if ($.browser.msie) {
  alert(
    "Please use Mozilla Firefox or Google Chrome. Microsoft Internet Explorer is not supported."
  );
}
...
function Refresh() {
  if (currentPage == "patient") RefreshPatient();
  else if (currentPage == "study") RefreshStudy();
  else if (currentPage == "series") RefreshSeries();
  else if (currentPage == "instance") RefreshInstance();
}
...
$(document).ready(function () {
  ...
  for (var i = 0; i < trees.length; i++) {
    ...
    $(trees[i]).bind("tree.click", function (event) {
      if (event.node.is_open) $(trees[i]).tree("closeNode", event.node, true);
      else $(trees[i]).tree("openNode", event.node, true);
    });
  }
...
function GetAuthorizationTokensFromUrl() {
  ...
  for (var i = 0; i < urlVariables.length; i++) {
    ...
    if (
      split.length == 2 &&
      (split[0] == "token" || split[0] == "auth-token" || split[0] == "authorization")
    ) {
...
function ParseDicomDate(s) {
  ...
  if (y == null || m == null || d == null || !isFinite(y) || !isFinite(m) || !isFinite(d)) {
    return null;
  }

  if (y < 1900 || y > 2100 || m < 0 || m >= 12 || d <= 0 || d >= 32) {
    return null;
  }
...
function FormatDicomDate(s) {
  if (s == undefined) return "No date";

  var d = ParseDicomDate(s);
  if (d == null) return "?";
  else return d.toString("dddd, MMMM d, yyyy");
}
...
function Sort(arr, fieldExtractor, isInteger, reverse) {
  ...
  arr.sort(function (a, b) {
    ...
    if (ta == undefined) ta = defaultValue;

    if (tb == undefined) tb = defaultValue;
...
function CompleteFormatting(node, link, isReverse, count) {
  if (count != null) {
    node = node.add($("<span>").addClass("ui-li-count").text(count));
  }

  if (link != null && link) {
    node = $("<a>").attr("href", link).append(node);

    if (isReverse) node.attr("data-direction", "reverse");
  }

  node = $("<li>").append(node);

  if (isReverse) node.attr("data-icon", "back");
  ...
  for (var i in tags) {
    if (tagsToIgnore.indexOf(i) == -1) {
      ...
      if (
        ...
        v = FormatDicomDate(v);
      } else if (i == STUDY_INSTANCE_UID || i == SERIES_INSTANCE_UID || i == SOP_INSTANCE_UID) {
...
function FormatPatient(patient, link, isReverse) {
  var node = $("<div>").append(
    $("<h3>").text(GetMainDicomTag(patient.MainDicomTags, PATIENT_NAME))
  );
...
function FormatStudy(study, link, isReverse, includePatient) {
  ...
  if (includePatient) {
    FormatMainDicomTags(node, study.PatientMainDicomTags, [PATIENT_NAME]);
  }

  FormatMainDicomTags(node, study.MainDicomTags, [STUDY_DESCRIPTION, STUDY_TIME]);
...
function FormatSeries(series, link, isReverse) {
  ...
  if (
    series.ExpectedNumberOfInstances == null ||
  ...
  node = $("<div>")
    ...
    .append($("<p>").append($("<em>").text("Status: ").append($("<strong>").text(series.Status))));
      ...
      if (s.Name != "") {
        ...
        $(".orthanc-name").append(
          $("<a>")
            ...
            .text("Powered by NeuroCare.AI")
            .append(" &raquo; ")
        );
      }

      // New in Orthanc 1.5.8
      if ("IsHttpServerSecure" in s && !s.IsHttpServerSecure) {
...
$("#lookup").live("pagebeforeshow", function () {
  ...
  target.append(
    $("<option>")
      .attr("value", GenerateDicomDate(-7) + "-")
      .text("Last 7 days")
  );
  target.append(
    $("<option>")
      .attr("value", GenerateDicomDate(-31) + "-")
      .text("Last 31 days")
  );
  target.append(
    $("<option>")
      .attr("value", GenerateDicomDate(-31 * 3) + "-")
      .text("Last 3 months")
  );
  target.append(
    $("<option>")
      .attr("value", GenerateDicomDate(-365) + "-")
      .text("Last year")
  );
...
$("#find-patients").live("pagebeforeshow", function () {
  GetResource(
    "/patients?expand&since=0&limit=" + (LIMIT_RESOURCES + 1) + "&full",
    function (patients) {
...
$("#find-studies").live("pagebeforeshow", function () {
  GetResource(
    "/studies?expand&since=0&limit=" + (LIMIT_RESOURCES + 1) + "&full",
    function (studies) {
    ...
    if (labels.length > 0) {
      ...
      for (var i = 0; i < labels.length; i++) {
        var removeButton = $("<button>")
          .text("X")
          .attr("title", 'Remove label "' + labels[i] + '"');
      ...
      GetResource("/patients/" + pageData.uuid + "?full", function (patient) {
        GetResource("/patients/" + pageData.uuid + "/studies?full", function (studies) {
          ...
          SetupAnonymizedOrModifiedFrom(
            "#patient-anonymized-from",
            patient,
            "patient",
            ANONYMIZED_FROM
          );
          SetupAnonymizedOrModifiedFrom(
            "#patient-modified-from",
            patient,
            "patient",
            MODIFIED_FROM
          );
        ...
        GetResource("/studies/" + series.ParentStudy + "?full", function (study) {
          GetResource("/patients/" + study.ParentPatient + "?full", function (patient) {
            GetResource("/series/" + pageData.uuid + "/instances?full", function (instances) {
              ...
              SetupAnonymizedOrModifiedFrom(
                "#series-anonymized-from",
                series,
                "series",
                ANONYMIZED_FROM
              );
              SetupAnonymizedOrModifiedFrom(
                "#series-modified-from",
                series,
                "series",
                MODIFIED_FROM
              );
        ...
        GetResource("/series/" + instance.ParentSeries + "?full", function (series) {
          GetResource("/studies/" + series.ParentStudy + "?full", function (study) {
            GetResource("/patients/" + study.ParentPatient + "?full", function (patient) {
              ...
              SetupAnonymizedOrModifiedFrom(
                "#instance-anonymized-from",
                instance,
                "instance",
                ANONYMIZED_FROM
              );
              SetupAnonymizedOrModifiedFrom(
                "#instance-modified-from",
                instance,
                "instance",
                MODIFIED_FROM
              );
...
function DeleteResource(path) {
  $.ajax({
    ...
    success: function (s) {
      ...
      if (ancestor == null) $.mobile.changePage("#lookup");
      else $.mobile.changePage("#" + ancestor.Type.toLowerCase() + "?uuid=" + ancestor.ID);
    },
  });
}
...
$("#instance-delete").live("click", function () {
  OpenDeleteResourceDialog("../instances/" + $.mobile.pageData.uuid, "Delete this instance?");
});

$("#study-delete").live("click", function () {
  OpenDeleteResourceDialog("../studies/" + $.mobile.pageData.uuid, "Delete this study?");
});

$("#series-delete").live("click", function () {
  OpenDeleteResourceDialog("../series/" + $.mobile.pageData.uuid, "Delete this series?");
});

$("#patient-delete").live("click", function () {
  OpenDeleteResourceDialog("../patients/" + $.mobile.pageData.uuid, "Delete this patient?");
});
      ...
      error: function () {
        GetResource("/instances/" + pageData.uuid + "/frames", function (frames) {
          if (frames.length == 1) {
            ...
            jQuery.slimbox(
              "../instances/" + pageData.uuid + "/preview?returnUnsupportedImage",
              "",
              {
            ...
            for (var i = 0; i < frames.length; i++) {
              images.push([
                "../instances/" +
                  ...
                  "/frames/" +
                  i +
                  "/preview?returnUnsupportedImage",
              ]);
            }
  ...
  if ($.mobile.pageData) {
    ...
    GetResource("/series/" + pageData.uuid, function (series) {
      GetResource("/series/" + pageData.uuid + "/instances", function (instances) {
        ...
        for (var i = 0; i < instances.length; i++) {
          images.push([
            "../instances/" + instances[i].ID + "/preview?returnUnsupportedImage",
...
function ChooseDicomModality(callback) {
  var clickedModality = "";
  var clickedPeer = "";
  var items = $("<ul>").attr("data-divider-theme", "d").attr("data-role", "listview");
...
$(".patient-attachment").live("click", function (e) {
  ...
  window.location.href =
    "../patients/" + $.mobile.pageData.uuid + "/attachments/" + e.target.id + "/data";
});

$(".study-attachment").live("click", function (e) {
  e.preventDefault(); //stop the browser from following
  window.location.href =
    "../studies/" + $.mobile.pageData.uuid + "/attachments/" + e.target.id + "/data";
});

$(".series-attachment").live("click", function (e) {
  e.preventDefault(); //stop the browser from following
  window.location.href =
    "../series/" + $.mobile.pageData.uuid + "/attachments/" + e.target.id + "/data";
});

$(".instance-attachment").live("click", function (e) {
  e.preventDefault(); //stop the browser from following
  window.location.href =
    "../instances/" + $.mobile.pageData.uuid + "/attachments/" + e.target.id + "/data";
});
...
$("#instance-anonymize").live("click", function () {
  OpenAnonymizeResourceDialog("../instances/" + $.mobile.pageData.uuid, "Anonymize this instance?");
});

$("#study-anonymize").live("click", function () {
  OpenAnonymizeResourceDialog("../studies/" + $.mobile.pageData.uuid, "Anonymize this study?");
});

$("#series-anonymize").live("click", function () {
  OpenAnonymizeResourceDialog("../series/" + $.mobile.pageData.uuid, "Anonymize this series?");
});

$("#patient-anonymize").live("click", function () {
  OpenAnonymizeResourceDialog("../patients/" + $.mobile.pageData.uuid, "Anonymize this patient?");
});
...
function AddJobField(target, description, field) {
  if (!(typeof field === "undefined")) {
    target.append($("<p>").text(description).append($("<strong>").text(field)));
  }
}
...
$("#jobs").live("pagebeforeshow", function () {
  $.ajax({
    ...
    success: function (jobs) {
      ...
      running = $("<li>").attr("data-role", "list-divider").text("Currently running");

      pending = $("<li>").attr("data-role", "list-divider").text("Pending jobs");

      inactive = $("<li>").attr("data-role", "list-divider").text("Inactive jobs");
  ...
  if ($.mobile.pageData) {
    ...
    $.ajax({
      ...
      success: function (job) {
        ...
        target.append(
          $("<li>").attr("data-role", "list-divider").text("General information about the job")
        );

        {
          block = $("<li>");
          for (var i in job) {
            if (i == "CreationTime" || i == "CompletionTime" || i == "EstimatedTimeOfArrival") {
              AddJobDateField(block, i + ": ", job[i]);
            } else if (i != "InternalContent" && i != "Content" && i != "Timestamp") {
        ...
        target.append($("<li>").attr("data-role", "list-divider").text("Detailed information"));
        ...
        if (job.State == "Running" || job.State == "Pending" || job.State == "Retry") {
